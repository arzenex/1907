...
function handleBoxClick(box, isWinner) {
    ...
    if (isWinner) {
        ...
        if (p) {
            p.innerText = "लो दिव्या खा लो गुलाब जामुन 😂";
...
function startSwipeToCut() {
    ...
    finalPrompt.style.backgroundColor = 'transparent';
    ...
    const startDrag = (e) => {
        const cakeGroup = document.getElementById('cake-group');
        if (!cakeGroup.contains(e.target)) return;
...
function triggerCakeExplosion() {
    ...
    // Note: wish.mp3 play is skipped here as per requirements

    // Animate the cake layers separating
...
function goStory() {
    music.pause();
    music.currentTime = 0;
    gsap.to("body", { opacity: 0, duration: 1, onComplete: () => { window.location.href = "story_divya.html"; } });
}
